...
{
	if (!Sentry::check()) return Redirect::to('admin/users/login');
});
...
View::composer('_layouts.default', function($view)
{
    ...
    $pages = Page::where('id', '!=', $homepage_id)->get();
	
	foreach($pages as $page)
	{		
		$arr['title'] = $page->title;
		$arr['link']  = URL::route('page', $page->slug);
		$arr['class'] = '';		
		if(Route::is('page') and Request::segment(1) == $page->slug)
			$arr['class'] = 'active';	
