...
class UserController extends BaseController {
	...
	public function getIndex()
	{
		...
		try
		{
		    ...
		    if ( Sentry::check())
			{
			    ...
			    return View::make('admin.users.index')->with($data);
			} else {
	...
	public function getShow($id)
	{
		try
		{
			...
			if ( $currentUser->hasAccess('admin') || $currentUser->getId() == $id)
			{
				...
				return View::make('admin.users.show')->with($data);
			} else {
	...
	//dd(Input::get('services'));
		...
		else 
		{
			...
			try {
				...
				$user_details->user_id		    = $data['userId'];
				...
				{
				    $m->to($data['email'])->subject('Welcome to Hour Exchange and thanks for signing up!');
				});
	...
	public function postLogin() 
	{
		...
		else 
		{
			...
			catch (Cartalyst\Sentry\Users\UserNotActivatedException $e)
			{
			    ...
			    Session::flash('error', 'You have not yet activated this account. <a href="/admin/users/resend">Resend actiavtion?</a>');
				return Redirect::to('admin/users/login')->withErrors($v)->withInput();
			}
	...
	public function getResetpassword() {
		...
		return View::make('admin.users.reset');
	}
	...
	public function postResetpassword () {
		...
		{
			// Validation has failed
			return Redirect::to('admin/users/resetpassword')->withErrors($v)->withInput();
		}
		else 
		{
			try
	...
	{
		//Show the Resend Activation Form
		return View::make('admin.users.resend');
	}
	...
	public function postResend()
	{
		...
		{
			// Validation has failed
			return Redirect::to('admin/users/resend')->withErrors($v)->withInput();
		}
		else 
		{
	...
	public function getReset($userId = null, $resetCode = null) {
		try
		{
		    ...
		    if ($user->attemptResetPassword($resetCode, $newPassword))
		    {
				...
				{
				    $m->to($data['email'])->subject('New Password Information');
				});
		    ...
		    else
		    {
			    ...
			    return Redirect::to('admin/users/resetpassword');
		    }
		}
		catch (Cartalyst\Sentry\Users\UserNotFoundException $e)
		{
	...
	public function getSuspend($id)
	{
		try
		{
			...
			{
				$data['user'] = Sentry::getUserProvider()->findById($id);
				return View::make('admin.users.suspend')->with($data);
			} else {
